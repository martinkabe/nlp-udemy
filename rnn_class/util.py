...
def get_wikipedia_data(n_files, n_vocab, by_paragraph=False):
    ...
    input_files = [f for f in os.listdir(
        prefix) if f.startswith('enwiki') and f.endswith('txt')]
    ...
    for sentence in sentences:
        if len(sentence) > 1:
            new_sentence = [
                idx_new_idx_map[idx] if idx in idx_new_idx_map else unknown for idx in sentence]
...
def get_stock_data():
    ...
    # T = length of series
    # make it T x D
    return np.array(all_prices).T, np.array(all_binary_targets).T
